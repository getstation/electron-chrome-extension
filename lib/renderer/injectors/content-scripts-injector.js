...
const setupContentScript = function (extensionId, worldId) {
  const context = {}
...
const injectContentScript = function (extensionId, worldId, url, code) {
  webFrame.executeJavaScriptInIsolatedWorld(worldId, code, url);
}
...
const addContentScript = function (extensionId, script) {
  ...
  const fire = script.js.map(function(js) {
    return injectContentScript(extensionId, worldId, js.url, js.code)
  });
...
const getManifest = () => ipcRenderer.sendSync('GET_RUNTIME_GET_MANIFEST_SYNC');
const manifest = getManifest();

const worldId = require('./isolated-worlds').getIsolatedWorldId(manifest.extensionId)
webFrame.setIsolatedWorldHumanReadableName(worldId, manifest.extensionName)
webFrame.setIsolatedWorldSecurityOrigin(worldId, `chrome-extension://${manifest.extensionId}`)

Object.keys(contentScripts).forEach(key => {
  const cs = contentScripts[key];
  if (cs.contentScripts) {
    setupContentScript(cs.extensionId, worldId)
