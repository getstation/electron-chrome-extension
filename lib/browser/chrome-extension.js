...
app.on('session-created', function (ses) {
  // by unregisterProtocol we are probably breaking some dev tools extensions
  // loaded via BrowserWindow.addDevToolsExtension
  // => we could fix that later 
  ses.protocol.unregisterProtocol('chrome-extension');
