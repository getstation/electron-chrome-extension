...
protocol.registerStandardSchemes([constants.DEFAULT_EXTENSION_PROTOCOL], { secure: true });
...
app.on('session-created', function (ses) {
  ...
  ses.protocol.registerBufferProtocol(constants.DEFAULT_EXTENSION_PROTOCOL, chromeExtensionHandler, function (error) {
    if (error) {
      console.error(`Unable to register ${constants.DEFAULT_EXTENSION_PROTOCOL} protocol: ${error}`)
    }
  })
})
